...
app.post('/addChild', async (req, res) => {
    try {
        ...
        // ใช้ birthdate จาก body หรือจะใช้วันที่ปัจจุบัน
        const birthdate = req.body.birthdate || req.body.birthday || new Date().toISOString().slice(0, 10);

        let sql = 'INSERT INTO child.CHILDREN(name, birthday, gender, parent_id) VALUES (?, ?, ?, ?)';
        let values = [
            req.body.name,
            birthdate,
            req.body.gender,
        ...
        db.query(sql, values, async (err, results, fields) => {
            ...
            if (req.body.parent_name && !req.body.parent_id && results.insertId) {
                const childId = results.insertId;
                let parentSql = 'INSERT INTO child.PARENTS(name) VALUES (?)';
                db.query(parentSql, [req.body.parent_name], async (parentErr, parentResults) => {
                    if (parentErr) {
                        console.error('Error creating parent:', parentErr);
                        return res.status(500).json({ error: false, data: results, msg: message, warning: "Child added but parent not linked" });
                    }

                    if (parentResults && parentResults.insertId) {
                        // อัปเดตข้อมูลเด็กให้เชื่อมกับผู้ปกครอง
                        let updateSql = 'UPDATE child.CHILDREN SET parent_id = ? WHERE child_ID = ?';
                        db.query(updateSql, [parentResults.insertId, childId], async (updateErr) => {
                            if (updateErr) {
                                console.error('Error linking child to parent:', updateErr);
                            }
                            res.json({ error: false, data: results, parent_data: parentResults, msg: message });
                        });
                    } else {
                        res.json({ error: false, data: results, msg: message });
                    }
